...
// Define types for the launch data
interface Launch {
  id: string;
  name: string;
  date_utc: string;
...
const sortByField = (field: keyof Launch | 'date_utc'): void => {
...
<template>
          ...
          <thead class="bg-glass">
            <tr>
              <th class="p-4 border-b border-r border-white border-opacity-20 cursor-pointer"
                @click="sortByField('name')">Rocket Name</th>
              <th class="p-4 border-b border-r border-white border-opacity-20 cursor-pointer"
                @click="sortByField('date_utc')">Launch Date</th>
              <th class="p-4 border-b border-r border-white border-opacity-20 cursor-pointer"
                @click="sortByField('crew')">Crew</th>
              <th class="p-4 border-b border-r border-white border-opacity-20 cursor-pointer"
                @click="sortByField('success')">Rocket Status</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="launch in launches" :key="launch.id" class="hover:bg-gray-50 cursor-pointer"
              @click="openModal(launch)">
              <td class="p-4 border border-white border-opacity-20">{{ launch.name }}</td>
              <td class="p-4 border border-white border-opacity-20">{{ new Date(launch.date_utc).toLocaleDateString() }}
              </td>
              <td class="p-4 border border-white border-opacity-20">{{ new Date(launch.date_utc).toLocaleTimeString() }}
              </td>
  ...
  <div v-if="showModal" class="fixed inset-0 min-h-screen bg-black bg-opacity-50 flex justify-center items-center p-4">
    <div class="bg-white opacity-90 w-full max-w-2xl p-8 rounded-lg shadow-lg relative">
      ...
      <p class="text-gray-700 mb-2"><strong>Details:</strong> {{ selectedLaunch?.details || 'No details available' }}
      </p>
